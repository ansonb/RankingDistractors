...
def build_vocab(dataset, pretrained_embeddings_path):
    ...
    if os.path.isfile(os.path.join(dataset,'vocab.pkl')):
        print('loading saved vocab...')
        # with open('{}/vocab.pkl'.format(dataset), 'rb', encoding='utf8') as fin:
        with open(os.path.join(dataset,'vocab.pkl'), 'rb') as fin:
        ...
        vocab['UNKNOWN'] = code
        code += 1
        filenames = [os.path.join(dataset,'train.data'), os.path.join(dataset,'valid.data'), os.path.join(dataset,'test.data') ]
                ...
                for i in range(2, 3):
                    ...
                    for word in words:
                        if word not in vocab:
                            ...
                            code += 1
    embd = None
    if os.path.isfile(os.path.join(dataset,'embd.pkl')):
        print('loading saved embd...')
        with open(os.path.join(dataset,'embd.pkl'), 'rb') as fin:
...
def loadTestSet(dataset, filename):
    testList = []
    for line in open('{}/{}'.format(dataset, filename), encoding='utf8'):
...
def read_raw(dataset):
    ...
    with open('{}/train.data'.format(dataset), 'r', encoding='utf8') as fin:
...
def read_alist(dataset):
    alist = []
    for line in open('{}/train.data'.format(dataset), encoding='utf8'):
        items = line.strip().lower().split(' ')
...
def read_alist_standalone(dataset, vocab, max_length, unk='<a>'):
    alist = []
    for line in open('{}/{}'.format(dataset, vocab), encoding='utf8'):
        items = line.strip().lower().split(' ')
